...
class KDTree
{
  ...
  private:
     ...
     */
    // quickselect
    int partition(vector<Point<Dim>>& newPoints, unsigned left, unsigned right, unsigned pivotIndex, int curDim);
    Point<Dim> select(vector<Point<Dim>>& newPoints, unsigned left, unsigned right, unsigned k, int curDim);
    void swap(Point<Dim>& first, Point<Dim>& second);
    KDTreeNode* recursiveBuild(vector<Point<Dim>>& list, unsigned left, unsigned right, int dim);


    void recursiveFind(KDTreeNode* curr, Point<Dim>& query, Point<Dim> & currBest, int dim) const;
    void copy__(KDTreeNode*& root, KDTreeNode*& other);
    void delete__(KDTreeNode* subroot);
};
