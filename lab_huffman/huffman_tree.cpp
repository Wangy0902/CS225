...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while (singleQueue.size() + mergeQueue.size() != 1) {
       TreeNode * remove_1 = removeSmallest(singleQueue, mergeQueue);
       TreeNode * remove_2 = removeSmallest(singleQueue, mergeQueue);
       TreeNode * internal = new TreeNode(remove_1->freq.getFrequency() + remove_2->freq.getFrequency());
       internal->left = remove_1;
       internal->right = remove_2;
